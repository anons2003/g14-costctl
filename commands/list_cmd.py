...
def run(args):
    ...
    label = " ".join(filters) if filters else "(no filter)"
    print(f"{args.type.upper()} {label} — {len(rows)} found:")
    print("-" * 78)
    for rid, kind, state, tags in rows:
        tag_text = " ".join(f"{k}={v}" for k, v in sorted(tags.items())) or "(no tags)"
        print(f"  {rid:<26} {kind:<14} {state:<13} {tag_text}")
